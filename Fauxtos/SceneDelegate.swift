...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let photoData = try! Data(contentsOf: Bundle.main.url(forResource: "Photos", withExtension: "json")!)
        let photos = try! JSONDecoder().decode([Photo].self, from: photoData)
        let mediaLibrary = MediaLibrary(photos: photos)

        // Create the SwiftUI view that provides the window contents.
        let contentView = ContentView().environmentObject(mediaLibrary)
